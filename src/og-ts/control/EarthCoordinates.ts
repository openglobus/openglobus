        ...
        if (this.position) {
            this.position.height =
                this.position.height > 10000 || this.position.height < -10000
                    ? 0
                    : this.position.height;
            const l = this._converter(this.position);
            const h = this.position.height > 0
            this._display.innerHTML =
                "Lat/Lon: " + l                 +
                " h(m): " +
                (this.position.height > 0
                    ? "~" + ((Math.round(this.position.height) / 1000) as any).toFixed(3) * 1000
                    : "-");
        } else {
