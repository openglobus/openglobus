...
/**
 * Computes the product of two matrices and store in the source.
 * @public
 * @param {og.Mat4} mx - Matrix to multiply.
 * @returns {og.Mat4} -
 */
Mat4.prototype.mulA = function (mx) {
    let d = this._m[0],
        e = this._m[1],
        g = this._m[2],
        f = this._m[3],
    ...
    this._m[0] = A * d + B * h + t * l + u * p;
    this._m[1] = A * e + B * i + t * o + u * r;
    this._m[2] = A * g + B * j + t * m + u * s;
    this._m[3] = A * f + B * k + t * n + u * a;
    this._m[4] = v * d + w * h + x * l + y * p;
    this._m[5] = v * e + w * i + x * o + y * r;
    this._m[6] = v * g + w * j + x * m + y * s;
    this._m[7] = v * f + w * k + x * n + y * a;
    this._m[8] = z * d + C * h + D * l + E * p;
    this._m[9] = z * e + C * i + D * o + E * r;
    this._m[10] = z * g + C * j + D * m + E * s;
    this._m[11] = z * f + C * k + D * n + E * a;
    this._m[12] = q * d + F * h + G * l + b * p;
    this._m[13] = q * e + F * i + G * o + b * r;
    this._m[14] = q * g + F * j + G * m + b * s;
    this._m[15] = q * f + F * k + G * n + b * a;
    return this;
};
