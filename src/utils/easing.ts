...
export class Easing {
    ...
    static QuadInOut: EasingFunction = (t: number) => (t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2);
    static CubicIn: EasingFunction = (t: number) => t * t * t;
    static CubicOut: EasingFunction = (t: number) => 1 - Math.pow(1 - t, 3);
    static CubicInOut: EasingFunction = (t: number) => (t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2);
    static QuartIn: EasingFunction = (t: number) => t * t * t * t;
    static QuartOut: EasingFunction = (t: number) => 1 - Math.pow(1 - t, 4);
    static QuartInOut: EasingFunction = (t: number) => (t < 0.5 ? 8 * t * t * t * t : 1 - Math.pow(-2 * t + 2, 4) / 2);
