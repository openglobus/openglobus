...
import { shadowPass } from "../../shaders/shadowPass/shadowPass";
...
export class ShadowPass {
    ...
    public dispose(): void {
        ...
        this._renderer.handler.removeProgram("shadowPass");
    }
    ...
    protected _computeScissor(
        ...
        const shadowCamera = shadowMap.depthCamera.camera;
        const shadowPV = shadowCamera.getProjectionViewRTEMatrix();
        this._tmpInverse.set(shadowPV).inverseTo(this._tmpInverse);
        const inv = this._tmpInverse._m;

        const shadowEye = shadowCamera.eye;
        const dx = shadowEye.x - mainEye.x;
        const dy = shadowEye.y - mainEye.y;
        const dz = shadowEye.z - mainEye.z;
    ...
    public apply(gBuffer: Framebuffer): void {
        ...
        const total = shadows.activeCount;

        if (total === 0) return;

        const h = r.handler;
        const gl = h.gl!;
        const sh = h.programs.shadowPass;
        const p = sh;
        const u = p.uniforms;
