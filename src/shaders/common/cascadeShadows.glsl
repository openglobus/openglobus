...
float getCascadeShadowDirectVisibility(vec3 rtcPos, vec3 normal, float viewDepth) {
    ...
    // Shadows stop at the last cascade, and stopping abruptly would draw a visible arc across the
    // ground. They are faded out over the tail of that cascade instead.
