...
void main() {
    ...
    if (isOutlinePass == 0) {
        ...
        // v_outline is a width in screen pixels
        float outlineWidth = clamp(v_outline, 0.0, sdfEdge * screenPxRange);
        float outlineOpacity = clamp((sd - 0.5) * screenPxRange + outlineWidth + 0.5, 0.0, 1.0);
