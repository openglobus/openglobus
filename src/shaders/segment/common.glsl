...
void applyPremultipliedSurfaceOpacity(inout vec4 color, float transitionOpacity, float frameOpacity) {
    float p = clamp(frameOpacity, 0.0, 1.0);
    float a = min(clamp(transitionOpacity, 0.0, 2.0), 1.0);
    color.rgb *= p * a;
    color.a *= a;
}

//void applyPremultipliedSurfaceOpacity(inout vec4 color, float transitionOpacity, float frameOpacity) {
//    color.rgb *= frameOpacity * transitionOpacity;
//    color.a *= transitionOpacity;
//}
